...
public final class TelegramPickerViewController: UIViewController {
    ...
    public struct MediaType: OptionSet {
        ...
        public static let photos = MediaType(1 << 0)
        public static let videos = MediaType(1 << 1)
        public static let camera = MediaType(1 << 2)
    }
    ...
    public required init(selection: @escaping TelegramSelection,
                         localizer: TelegramPickerResourceProvider,
    ...
    func setupCameraStream(_ completionHandler: @escaping (Camera.PreviewStream?) -> Void) {
    ...
    func checkCameraState(completionHandler: @escaping (Camera.PreviewStream?) -> Void) {
        ...
        collectionView.performBatchUpdates({
            changesToApply.forEach { (change) in
                ...
                case .inserted(let asset, at: let idx):
                    ...
                    if let item = StreamItem(asset: asset) {
    ...
    private func handleSingleSelectionUserChoice(item: StreamItem, at indexPath: IndexPath) {
        ...
        var selectionItem: TelegramSelectionType?
...
extension TelegramPickerViewController: UICollectionViewDataSource {
    ...
    private func preheatNextItems(at indexPath: IndexPath) {
        ...
        let preheatEntries = assetsToPreheat.map({
            Assets.PreheatRequest.Entry(asset: $0, size: sizeForAsset(asset: $0))
        })
        let request = Assets.PreheatRequest(entries: preheatEntries)
...
private extension TelegramPickerViewController {
    ...
    func updateAssetSelection<T>(cell: CollectionViewCustomContentCell<T>? = nil,
                                 asset: PHAsset,
...
extension TelegramPickerViewController: CollectionViewCustomContentCellDelegate {
    func collectionViewCustomContentCell<T>(cell: CollectionViewCustomContentCell<T>,
                                            didTapOnSelection button: UIButton) where T: UIView {

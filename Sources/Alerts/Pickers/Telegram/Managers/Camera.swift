...
public final class Camera {
    ...
    public static func requestAccess(_ requestGranted: @escaping (Bool) -> Void) {
    ...
    public final class PreviewStream: Equatable {
        ...
        public static func create(_ completionHandler: @escaping (StreamResult) -> Void) {
            ...
            queue.async {
                ...
                let stream = PreviewStream(device: device, input: input, session: session, queue: queue)
        ...
        private static func createQueue() -> DispatchQueue {
            return DispatchQueue(label: "CameraQueue \(arc4random())",
                                      qos: .background,
                                      attributes: [],

...
extension UIAlertController {
    ...
    public func addContactsPicker(localizer: TelegramPickerResourceProvider? = nil,
                                  selection: @escaping ContactsPickerViewController.Selection) {
        let selection: ContactsPickerViewController.Selection = selection
        var contact: Contact?

        let addContact = UIAlertAction(title: localizer?.localized(buttonType: .addContact) ?? "Add Contact", style: .default) { _ in
            selection(contact)
        }
...
public final class ContactsPickerViewController: UIViewController {
    ...
    public required init(selection: Selection?) {
    ...
    func checkStatus(completionHandler: @escaping ([String: [CNContact]]) -> Void) {
        ...
        case .denied, .restricted:
            ...
            let productName = Bundle.main.appName
            let alert = UIAlertController(title: "Permission denied",
                                          message: "\(productName) does not have access to contacts. Please, allow the application to access to your contacts.",

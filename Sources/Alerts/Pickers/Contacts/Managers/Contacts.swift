...
public enum Contacts {
    ...
    @available(iOS 10.0, *)
    public static func fetchContacts(ContactsSortorder sortOrder: CNContactSortOrder,
                                     completionHandler: @escaping (_ result: FetchResults) -> Void) {
...
public enum Telephone {
    ...
    public static func isCapableToCall(completionHandler: @escaping (_ result: Bool) -> Void) {
        if UIApplication.shared.canOpenURL(NSURL(string: "tel://")! as URL) {
            ...
            if let mnc: String = CTTelephonyNetworkInfo().serviceSubscriberCellularProviders?["home"]?.mobileNetworkCode,
                  !mnc.isEmpty {

...
private struct AssetManager: Hashable {
    ...
    internal let manager = PHCachingImageManager()
...
public struct Assets {
    ...
    public struct PreheatRequest {
        ...
        public var sizedAssetGroups: [SizedAssetGroup] {
            ...
            func provideGroup(size: CGSize) -> SizedAssetGroup {
                ...
                let newGroup = SizedAssetGroup(size: size)
    ...
    public static func cacheManager(size: CGSize) -> PHCachingImageManager {
        ...
        let newEntry = AssetManager(size: size)
    ...
    public static func fetchOriginal(_ completion: @escaping (OriginalFetchResults) -> Void ) {
        ...
        guard PHPhotoLibrary.authorizationStatus() == .authorized else {
            let error: NSError = NSError(domain: "PhotoLibrary Error",
                                         code: 1,
                                         userInfo: [NSLocalizedDescriptionKey: "No PhotoLibrary Access"])
    ...
    public static func preheat(request: PreheatRequest) {
        ...
        for group in request.sizedAssetGroups {
            ...
            manager.startCachingImages(for: group.assets,
                                       targetSize: group.size,
                                       contentMode: .aspectFill,
                                       options: defaultImageRequestOptions)
        }
    }

    public static func resolve(asset: PHAsset,
    ...
    public static func resolveVideo(asset: PHAsset,
                                    size: CGSize = PHImageManagerMaximumSize,
                                    cacheAllowed: Bool = true,
                                    completion: @escaping (_ image: URL?) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
    ...
    public static func resolve(assets: [PHAsset],
                               size: CGSize = CGSize(width: 720, height: 1280),
                               completion: @escaping (_ images: [UIImage]) -> Void) -> [UIImage] {

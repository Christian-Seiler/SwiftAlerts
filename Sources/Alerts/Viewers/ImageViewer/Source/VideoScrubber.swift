...
open class VideoScrubber: UIControl {
    ...
    public var onDidChangePlaybackState: ((PlaybackState) -> Void)?
    ...
    weak var player: AVPlayer? {
        ...
        didSet {
            ...
            if let player = player {
                ...
                ///TIMER
                periodicObserver = player.addPeriodicTimeObserver(forInterval: CMTime(value: 1, timescale: 1),
                                                                  queue: nil,
    ...
    func setup() {
        ...
        timeLabel.attributedText = NSAttributedString(string: "--:--",
    ...
    @objc func updateCurrentTime() {
        ...
        if let duration = self.duration, self.duration != nil {
            ...
            timeLabel.attributedText = NSAttributedString(string: timeString,
                                                          attributes: [NSAttributedString.Key.foregroundColor: self.tintColor!,
                                                                       NSAttributedString.Key.font: UIFont.systemFont(ofSize: 12)])
        } else {
            timeLabel.attributedText = NSAttributedString(string: "--:--",

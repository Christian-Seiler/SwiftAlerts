...
final class ContactTableViewCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        textLabel?.frame = CGRect(origin: CGPoint(x: textLabelMarginLeft,
                                                  y: textLabel?.frame.origin.y ?? 0),
                                  size: textLabel?.bounds.size ?? CGSize.zero)
        detailTextLabel?.frame = CGRect(origin: CGPoint(x: textLabelMarginLeft,
                                                        y: detailTextLabel?.frame.origin.y ?? 0),
                                        size: detailTextLabel?.bounds.size ?? CGSize.zero)
    }

...
extension Date {
    ...
    public var minute: Int {
        ...
        set {
            self = Date(calendar: calendar, timeZone: timeZone, era: era, year: year, month: month, day: day,
...
extension Date {
    ...
    func end(of component: Calendar.Component) -> Date? {
        ...
        case .second:
            ...
            guard let after = calendar.date(from: calendar.dateComponents([.year,
                                                                           .month,
                                                                           .day,
                                                                           .hour,
                                                                           .minute,
        ...
        case .minute:
            ...
            guard let after = calendar.date(from: calendar.dateComponents([.year, .month, .day, .hour, .minute],
                                                                          from: date)) else {
                return nil
            }
        ...
        case .hour:
            ...
            guard let after = calendar.date(from: calendar.dateComponents([.year, .month, .day, .hour],
                                                                          from: self)) else {
                return nil
            }
            date = after.adding(.second, value: -1)
        ...
        case .weekOfYear, .weekOfMonth:
            var date = self
            guard let beginningOfWeek = calendar.date(from: calendar.dateComponents([.yearForWeekOfYear, .weekOfYear],
                                                                                    from: self)) else {
                return nil
            }
...
extension Date {
    ...
    static func randomWithinDaysBeforeToday(_ days: Int) -> Date {
        ...
        let rDay = UInt32.random(in: 0..<UInt32(days))
        let rHour = UInt32.random(in: 0..<24)
        let rMinute = UInt32.random(in: 0..<60)
        let rSecond = UInt32.random(in: 0..<60)

        var offsetComponents = DateComponents()
        offsetComponents.day = Int(rDay) * -1
        offsetComponents.hour = Int(rHour)
        offsetComponents.minute = Int(rMinute)
        offsetComponents.second = Int(rSecond)
    ...
    static func random() -> Date {
        let randomTime = TimeInterval(UInt32.random(in: 0..<UInt32.max))

...
open class GalleryViewController: UIPageViewController, ItemControllerDelegate {
    ...
    fileprivate var thumbnailsButton: UIButton?
    fileprivate var deleteButton: UIButton?
    fileprivate var sendButton: UIButton? = UIButton.sendButton()
    fileprivate let scrubber = VideoScrubber()
    fileprivate let scrubberBackgroundView = UIVisualEffectView(effect: UIBlurEffect(style: .dark))

    fileprivate weak var initialItemController: ItemController?

    // MARK: - LOCAL STATE
    // represents the current page index, updated when the root view of the view controller representing the page stops animating inside
    // visible bounds and stays on screen.
    ...
    public init(startIndex: Int, itemsDataSource: GalleryItemsDataSource, itemsDelegate: GalleryItemsDelegate? = nil, displacedViewsDataSource: GalleryDisplacedViewsDataSource? = nil, configuration: GalleryConfiguration = []) {
        ...
        /// Only those options relevant to the paging GalleryViewController are explicitly handled here,
        /// the rest is handled by ItemViewControllers
        ...
        pagingDataSource = GalleryPagingDataSource(itemsDataSource: itemsDataSource,
    ...
    fileprivate func configureOverlayView() {
        ...
        overlayView.bounds.size = UIScreen.main.bounds.insetBy(dx: -UIScreen.main.bounds.width / 2,
                                                               dy: -UIScreen.main.bounds.height / 2).size
        overlayView.center = CGPoint(x: (UIScreen.main.bounds.width / 2),
    ...
    fileprivate func layoutFooterView() {
        ...
        case .pinBoth(let marginBottom, let marginLeft, let marginRight):
            ...
            footer.sizeToFit()
            footer.frame.origin = CGPoint(x: marginLeft,
                                          y: self.view.bounds.height - footer.bounds.height - marginBottom - defaultInsets.bottom)
        ...
        case .pinRight(let marginBottom, let marginRight):
            ...
            footer.frame.origin = CGPoint(x: self.view.bounds.width - marginRight - footer.bounds.width,
                                          y: self.view.bounds.height - footer.bounds.height - marginBottom - defaultInsets.bottom)
        }
    }
    ...
    fileprivate func layoutScrubber() {
        ...
        scrubberBackgroundView.bounds = CGRect(origin: CGPoint.zero, size: CGSize(width: self.view.bounds.width,
                                                                                  height: 40 + marginsBottom + marginsTop))
